...
def eme(x, window_size):
    ...
    x = x[:int(blocksize_x*k2), :int(blocksize_y*k1)]
...
def uiqm(x):
    ...
    uiqm = (1*uicm(x)) + (1*uism(x)) + (1*uiconm(x))
    return uiqm
...
#UIComM
def contrast(image):
 #   image=cv2.imread("226_imag_.png")
    c = cv2.cvtColor(image, cv2.COLOR_BGR2YUV)
    (Y,u,n)=cv2.split(c)
...
#Calculation
def uiqm(image,c1,c2,c3):
    uiqm = c1*uicm(image) +c3*contrast(image) + c2*variance_of_laplacian(image)
    return(uiqm)"""
